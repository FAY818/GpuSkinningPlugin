...
public class GPUSkinningPlayerMono : MonoBehaviour
{
    [HideInInspector]
    [SerializeField]
    private GPUSkinningAnimation anim = null; // 采样的动画文件

    [HideInInspector]
    [SerializeField]
    private Mesh mesh = null; // 采样网格

    [HideInInspector]
    [SerializeField]
    private Material mtrl = null; // 采样贴图

    [HideInInspector]
    [SerializeField]
    private TextAsset textureRawData = null; // 采样贴图（二进制）
    
    [HideInInspector]
    [SerializeField]
    private TextAsset textureBindRawData = null; // 采样贴图（二进制）

    [HideInInspector]
    [SerializeField]
    private int defaultPlayingClipIndex = 0; // 默认动画片段索引
    ...
    public void Init()
    {
        ...
        if (anim != null && mesh != null && mtrl != null && textureRawData != null && textureBindRawData != null)
        {
            ...
            else
            {
                ...
                res.textureBind = GPUSkinningUtil.CreateTexture2D(textureBindRawData, anim);
