...
public class GPUSkinningSampleSetting : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            UpdateBoneSampleProcess();
        }
        else if (animType == GPUSkinningAnimType.Vertices)
        {
            UpdateVertexSampleProcess();
        }
    }
    ...
    // 处理骨骼动画相关流程
    private void UpdateBoneSampleProcess()
    {
        if (!isSampling)
        {
            return;
            ...
            if (!string.IsNullOrEmpty(savePath))
            {
                ...
                else
                {
                    ...
                    if (samplingClipIndex == 0)
                    {
                        ...
                        CreatePrefab(dir, prefabFileName, dataFileName, meshFileName, mtrlFileName,
                            textureRawDataFileName, textureRawBindDataFileName);
                    }
    ...
    // 处理顶点动画相关流程
    private void UpdateVertexSampleProcess()
    {
        if (!isSampling)
        {
            return;
        ...
        if (samplingFrameIndex >= totalFrams)
        {
            ...
            if (!string.IsNullOrEmpty(savePath))
            {
                ...
                else
                {
                    ...
                    string dir = "Assets" + savePath.Substring(Application.dataPath.Length); // 根目录

                    // ScriptObj相关
                    string savedAnimPath = dir + "/GPUSKinning_AnimVert_" + animName + ".asset";
                    ...
                    if (samplingClipIndex == 0)
                    {
                        Mesh newMesh = CreateNewVertMesh(smr.sharedMesh, "GPUSkinning_MeshVert");
                        if (savedMesh != null)
                        {
                            newMesh.bounds = savedMesh.bounds;
                        }
                        ...
                        // string prefabFileName = "GPUSKinning_PrefabVert_" + animName + ".prefab";
                        // string dataFileName = "GPUSKinning_AnimVert_" + animName + ".asset";
                        // string meshFileName = "GPUSKinning_MeshVert_" + animName + ".asset";
                        // string mtrlFileName = "GPUSKinning_MaterialVert_" + animName + ".mat";
                        // string textureRawDataFileName = "GPUSKinning_TextureVert_" + animName + ".bytes";
                        // CreatePrefab(dir, prefabFileName, dataFileName, meshFileName, mtrlFileName,
                        //     textureRawDataFileName, null);
                    }
    ...
    /// 创建预制体
    /// </summary>
    private void CreatePrefab(string savePath, string prefabFileName, string dataFileName, string meshFileName,
        string mtrlFileName, string textureRawDataFileName, string textureRawBindDataFileName)
    {
    ...
    /// 
    /// </summary>
    private Mesh CreateNewVertMesh(Mesh mesh, string meshName)
    {
        // 当前Mesh的数据
        Vector3[] normals = mesh.normals;
        Vector4[] tangents = mesh.tangents;
    ...
    private void CreateVertexTexture(string dir, GPUSkinningAnimation gpuSkinningAnim, Mesh mesh)
    {
        ...
        string savedPathBind = dir + "/GPUSKinning_VertexTexture_" + animName + ".bytes";
        using (FileStream fileStream = new FileStream(savedPathBind, FileMode.Create))
        {
